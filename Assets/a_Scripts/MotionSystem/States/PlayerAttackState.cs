...
namespace RPG.MotionSystem.States
{
    public class PlayerAttackState : FSMState<PlayerMotion>
    {
        ...
        public override void OnUpdate(PlayerMotion owner)
        {
            ...
            if (owner.Timer.GetElapsed(StringConstants.TimerName.LightAttackTime1) >= 1f && _comboIndex == 0)
            {
                Debug.Log("Attack1 interrupted");
                _canTransition = true;
            }
            else if (owner.Timer.GetElapsed(StringConstants.TimerName.LightAttackTime2) >= 1f && _comboIndex == 1)
            {
                Debug.Log("Attack2 interrupted");
                _canTransition = true;
            }
            else if (owner.Timer.GetElapsed(StringConstants.TimerName.LightAttackTime3) >= 1f && _comboIndex == 2)
            {
